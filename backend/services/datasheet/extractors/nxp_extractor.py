...
Extracts IC specification data from NXP PDF datasheets.
...
class NXPExtractor(DatasheetExtractor):
    """Extractor for NXP Semiconductors PDF datasheets."""
    
    def extract(self, pdf_path: Path) -> List[Dict]:
        """
        Extract IC specification data from NXP PDF datasheet.
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    for table in tables:
                        ...
                        if any(keyword in header_text for keyword in [
                            ...
                            "type number", "ordering information"
                        ]):
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                if any(keyword in header_text for keyword in [
                    ...
                    "absolute maximum ratings", "recommended operating", "limiting values"
                ]):
                    for row in table[1:]:
                        ...
                        if "temperature" in first_cell and ("operating" in first_cell or "range" in first_cell or "ambient" in first_cell):
                            for cell in row[1:]:
                                if cell:
    ...
    def _extract_voltage_range(self, text: str) -> Optional[Dict]:
        """Extract voltage min/max from text like '1.8 to 3.6' or '1.8V to 3.6V'."""
        if not text:
            return None
        ...
        # Single value pattern (e.g., "3.6" for max voltage)
    ...
    def _extract_temperature_range(self, text: str) -> Optional[Dict]:
        """Extract temperature min/max from text like '-40 to +85°C' or '-40 to 125'."""
        if not text:
            return None
    ...
    def _extract_from_table(
        ...
        for row in table[1:]:
            ...
            # Extract part number/type number
            ...
            variant = {
                ...
                "description": f"NXP {part_number} - {package_type or 'Unknown Package'}",
    ...
    def _find_column_indices(self, header_row: List) -> Dict[str, int]:
        ...
        NXP datasheets typically use "Type number", "Package", columns.
        """
        indices = {}
        ...
        for idx, cell in enumerate(header_row):
            ...
            # Part number/Type number column
            if any(keyword in cell_lower for keyword in ["type number", "part number", "order", "device"]) and "part_number" not in indices:
    ...
    def _extract_pin_count(self, row: List, col_index: Optional[int]) -> Optional[int]:
        ...
        # Try to extract from package name (e.g., "HVQFN20", "TSSOP20", "DIP20")
        for cell in row:
            if cell:
                cell_str = str(cell).upper()
                # Look for patterns like HVQFN20, TSSOP20, etc.
                match = re.search(r'(HVQFN|VQFN|QFN|TSSOP|SOIC|DIP|QFP|LQFP|SSOP|MSOP|SO)(\d+)', cell_str)
                if match:
                    try:
    ...
    def _create_basic_entry(self, pdf_path: Path, voltage_specs: Dict, temp_specs: Dict) -> Dict:
        ...
        # Remove "NXP_" prefix if present
        if part_number.startswith("NXP_"):
